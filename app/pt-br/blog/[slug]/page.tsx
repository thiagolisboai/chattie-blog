      ...
      <main id="main-content" style={{ maxWidth: 760, margin: '0 auto', padding: '3rem 1.5rem 5rem' }}>
        ...
        <footer style={{ marginTop: '4rem', borderTop: '2px solid #000', paddingTop: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '2.5rem' }}>
            {post.tags?.map((tag) => (
              <Link key={tag} href={`/pt-br/blog/tag/${tag.toLowerCase().replace(/\s+/g, '-')}`} className="tag-pill">
                #{tag}
              </Link>
            ))}
          </div>
          ...
          {author && (
            <div className="author-card">
              ...
              <div className="author-card-body">
                ...
                <Link href={`/pt-br/blog/autor/${author.slug}`} style={{ fontFamily: "'Sherika', sans-serif", fontWeight: 900, fontSize: '1.05rem', marginBottom: '0.15rem', display: 'block', textDecoration: 'none', color: '#000', letterSpacing: '-0.01em' }} className="hover:text-teal">
                  {author.name}
                </Link>
                <p style={{ fontSize: '0.78rem', color: '#2F6451', fontWeight: 600, marginBottom: '0.6rem' }}>{author.rolePt}</p>
                <p style={{ fontSize: '0.875rem', color: '#444', lineHeight: 1.65, margin: 0 }}>{author.bioPt}</p>
                <a
